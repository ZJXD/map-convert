...
(function() {
  /**
     ...
     */
  var T
  var baidu = T = baidu || { version: '1.3.8' };
  ...
  (function() {
    ...
    baidu.dom._styleFilter.filter = function(key, value, method) {
      for (var i = 0, filters = baidu.dom._styleFilter; i < filters[i].length; i++) {
        if (filters[i][method]) {
          const filter = filters[i][method]
    ...
    baidu.dom.getStyle = function(element, key) {
      ...
      // 在取不到值的时候，用fixer进行修正
      if (!value) {
        const fixer = dom._styleFixer[key]
        if (fixer) {
    ...
    baidu.dom.getPosition = function(element) {
      ...
      var viewport = (browser.ie && !browser.isStrict) ? doc.body : doc.documentElement
      var parent
      var box
      ...
      if (element.getBoundingClientRect) { // IE and Gecko 1.9+
        ...
        var htmlBorderLeftWidth = parseInt(getStyle(htmlDom, 'borderLeftWidth'))
        var htmlBorderTopWidth = parseInt(getStyle(htmlDom, 'borderTopWidth'))
    ...
    baidu.lang.Class.prototype.dispatchEvent = function(event, options) {
      ...
      let i; const t = this.__listeners; let p = event.type
  ...
  TextIconOverlay.prototype._setupDefaultStyles = function() {
    ...
    for (var i = 0; i < sizes.length; i++) {
      this._styles.push({
        ...
        size: new BMap.Size(sizes[i], sizes[i])
      })
    }// for循环的简洁写法
  }
